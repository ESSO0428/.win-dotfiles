...
map("<leader>m", "<cmd>call InterestingWords('n')<cr>", "InterestingWords('n')")
vim.keymap.set("v", "<leader>m", ":call InterestingWords('v')<cr>", { desc = "InterestingWords('v')" })
map("<leader>M", "<cmd>call UncolorAllWords()<cr>", "UncolorAllWords")
map("n", "<cmd>call WordNavigation(1)<cr>", "WordNavigation(1)")
map("N", "<cmd>call WordNavigation(0)<cr>", "WordNavigation(0)")
...
function telescope_interestingwords_selected(use_stored_words)
  ...
  local function get_interesting_word_color(word)
    local colors = vim.g.interestingWordsGUIColors
      or { "#aeee00", "#fa3e2d", "#2d93fa", "#b970e0", "#ffa724", "#fc7cc5" }
  ...
  pickers
    .new({}, {
      ...
      attach_mappings = function(prompt_bufnr, map)
        ...
        map("n", "<a-,>", function()
          navigate_occurrences "prev"
        end)
        map("n", "<a-.>", function()
          navigate_occurrences "next"
        end)
        map("n", "<", function()
          navigate_occurrences "prev"
        end)
        ...
        map("n", "<Up>", function()
          move_selection_previous(prompt_bufnr)
        end)
        map("n", "<Down>", function()
          move_selection_next(prompt_bufnr)
        end)
        map("i", "<Up>", function()
          move_selection_previous(prompt_bufnr)
        end)
        ...
        actions.select_default:replace(function()
          ...
          vim.defer_fn(function()
            ...
            vim.notify(
              "Jumped to occurrence " .. current_occurrence_index .. " of " .. #occurrences .. " for: " .. selected_word,
              vim.log.levels.INFO
            )
          end, 10)
        end)
...
map("<leader>si", "<cmd>lua telescope_interestingwords_selected(false)<cr>", "Search interestingwords")
